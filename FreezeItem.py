...
class FreezeItem(Target):
    ...
    def apply_effect(self, player, other=None):
        # پخش صدای فریز و اعمال اثر کند شدن روی نشانگر حریف (other)، نه خود بازیکنی که هدف را زده
        self.freeze_sound.play()
        other.cursor.speed = 2  # کاهش سرعت نشانگر حریف
        other.cursor.freeze_timer = 180  # مدت زمان اثر فریز (بر حسب فریم؛ با update در Cursor کم می‌شود)